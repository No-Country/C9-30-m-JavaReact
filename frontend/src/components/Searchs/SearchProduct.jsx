...
import { getCampaignProducts} from "../../js/search";
...
function SearchProducts() {
  ...
  const resultsFilter = !search
    ? campanas
    : campanas.filter((date) =>
        date.name.toLowerCase().includes(search.toLocaleLowerCase())
      );


  useEffect(() => {
    async function run() {
      const campaignsProducts = await getCampaignProducts();
      setCampanas(campaignsProducts);
    }
    run();
  }, []);
  ...
  return (
    <div className="search d-flex ">
      <div className="containerSearch">
        ...
        <h4 className="mt-4 mb-5">Productos</h4>
